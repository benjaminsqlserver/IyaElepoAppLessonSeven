...
{
    public partial class EditAspNetRoleComponent : ComponentBase
    {
        ...
        {
            var conDataGetAspNetRoleByIdResult = await ConData.GetAspNetRoleById($"{Id}");
            aspnetrole = conDataGetAspNetRoleByIdResult;
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(IyaElepoApp.Models.ConData.AspNetRole args)
        {
            try
            {
                var conDataUpdateAspNetRoleResult = await ConData.UpdateAspNetRole($"{Id}", aspnetrole);
                DialogService.Close(aspnetrole);
            }
            catch (System.Exception conDataUpdateAspNetRoleException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update AspNetRole" });
            }
        }
