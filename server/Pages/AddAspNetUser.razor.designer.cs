...
{
    public partial class AddAspNetUserComponent : ComponentBase
    {
        ...
        {
            aspnetuser = new IyaElepoApp.Models.ConData.AspNetUser(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(IyaElepoApp.Models.ConData.AspNetUser args)
        {
            try
            {
                var conDataCreateAspNetUserResult = await ConData.CreateAspNetUser(aspnetuser);
                DialogService.Close(aspnetuser);
            }
            catch (System.Exception conDataCreateAspNetUserException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new AspNetUser!" });
            }
        }
