...
{
    public partial class AddAspNetUserTokenComponent : ComponentBase
    {
        ...
        {
            aspnetusertoken = new IyaElepoApp.Models.ConData.AspNetUserToken(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(IyaElepoApp.Models.ConData.AspNetUserToken args)
        {
            try
            {
                var conDataCreateAspNetUserTokenResult = await ConData.CreateAspNetUserToken(aspnetusertoken);
                DialogService.Close(aspnetusertoken);
            }
            catch (System.Exception conDataCreateAspNetUserTokenException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new AspNetUserToken!" });
            }
        }
